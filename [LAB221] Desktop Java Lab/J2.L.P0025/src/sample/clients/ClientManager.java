...
public class ClientManager {
    ...
    private HashMap<Integer, String> listCounter;
    private HashMap<Integer, Boolean> listYourTurn;
    private HashMap<Integer, String> listYourText;
    ...
    public void sendInvitation(String data){
        client.sendData(KeyWord.INIVTE+data);
    }
    
    public void respondToInvitation(String data){
        String str[]=data.split("[:-]");
        String counter_player=str[3];
        int size=Integer.parseInt(str[2]);
        String[] options = {"Accept", "Reject"};
        int x=JOptionPane.showOptionDialog(homeUI, "Do you want to play caro ("+size+"x"+size+") with "+counter_player+"?", "Invite", JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, options, options[1]);
        System.out.println(x);
        if(x==0){           
            int roomID=createRoomID();
            listCounter.put(roomID, counter_player);          
            GameUI gameUI=new GameUI(this,roomID, size); // loi tu day           
            ...
            client.sendData(KeyWord.RESPONE_INVITATION+"OK-"+str[1]+"-"+str[3]+"-"+textDisplayed+"-"+roomID+"-"+size); //"response:
        }else{
            client.sendData(KeyWord.RESPONE_INVITATION+"NO-"+str[1]+"-"+str[3]);
        }
    }
    ...
    public void receiveRespone(String data){
        if(data.contains("OK")){
            String str[]=data.split("-");
            String turn=str[3];
            int size=Integer.parseInt(str[5]);
            String textDisplayed=(turn.equals("X"))?"O":"X";

            int roomID=Integer.parseInt(str[4]);
            GameUI gameUI=new GameUI(this, roomID, size);
    ...
    public void sendRequestUpdateUI(String data){
        String str[]=data.split("[:-]");
        String roomID=str[2];
        String msg=data+"-"+listCounter.get(Integer.parseInt(roomID)); //"your-turn:{roomID}-{positionY}-{positionX}-{counterPlayer}"
        client.sendData(msg);
    ...
    public void receiveRequestUpdateUI(String data){
        ...
        int positionY=Integer.parseInt(str[3]);
        int positionX=Integer.parseInt(str[4]);
        String textDisplayed=listYourText.get(Integer.parseInt(roomID));
        (gameUI.getArr())[positionY][positionX]=textDisplayed.equals("X")?1:0;
        (gameUI.getListLb())[positionY][positionX].setText(textDisplayed.equals("X")?"O":"X");
