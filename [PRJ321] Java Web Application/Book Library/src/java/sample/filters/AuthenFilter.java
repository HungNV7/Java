...
public class AuthenFilter implements Filter {
    ...
    public AuthenFilter() {
        ...
        user.add("RemoveItemController");
        user.add("DetailController");
        user.add("SubmitCartController");
        ...
        admin.add("listOrder.jsp");
        admin.add("newBook.jsp");
        admin.add("updateBook.jsp");
        admin.add("LoginController");
        admin.add("LogoutController");
        admin.add("AddNewBookController");
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            if (uri.contains(".png") || uri.contains(".jpg")) {
                ...
                if (uri.contains("login.jsp") || uri.contains("home.jsp") || uri.contains("MainController") || uri.contains("LoginController") || uri.contains("LogoutController")
                        || uri.contains("search.jsp") || uri.contains("detail.jsp") || uri.contains("DetailController")
                ...
                if (session == null || session.getAttribute("USER") == null) {
                    if (uri.contains("register.jsp") || uri.contains("RegisterController") || uri.contains("viewCart.jsp") || uri.contains("history.jsp") || uri.contains("AddToCartController")) {
                        chain.doFilter(request, response);
                    } else {
                        rs.sendRedirect(HOME_PAGE);
                    }
                } else {
                    UserDTO dto = (UserDTO) session.getAttribute("USER");
                    String roleID = dto.getRoleID();
                    if (roleID.equals("RL001") && admin.contains(resource)) {
                        chain.doFilter(request, response);
                    } else if (roleID.equals("RL002") && user.contains(resource)) {
                        chain.doFilter(request, response);
                    } else {
                        rs.sendRedirect(HOME_PAGE);
                    }
            ...
            log("Error in Filter: " + e.toString());
        }
