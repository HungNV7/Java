...
public class CheckoutServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            cartDAO.addOrder(cart, user);
            String orderID = cartDAO.getOrderID();
            cart.setId(orderID);

            RoomDAO roomDAO = new RoomDAO();
            roomDAO.addOrderDetail(cart);

            String email = request.getParameter("email");
            String confirmLink = "http://localhost:8080/Hotel_Booking/confirmBook?order=" + orderID + "&user=" + user.getUserID();
